...
| `get_disclosures` | one row per paper, tag and passage | month, field, bucket, tag, arxiv_id |
...
| `get_paper` | one paper: outcome, tags, passages, tools | arxiv_id |
| `get_examined` | the set of all papers, per month and field | month, field |
...
If a paper does contain an AI-use acknowledgment, the `tag` will return what types of AI-use acknowledgments the paper contains, and the `passage` associated with the tag is a verbatim quote from the paper justifying the inclusion of that tag. 
...
Licensed under [CC BY 4.0](LICENSE). Passages are quoted from arXiv paper text and are not covered under this license. 
